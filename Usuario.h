...
public:
    ...
    Usuario(size_t _id, const std::string& _nombre,
        const std::string& _correo,
        const std::string& _password,
        const std::string& _rol)
        : id(_id), nombre(_nombre), correo(_correo),
        password(_password), rol(_rol)
    {
    }
    ...
    static std::vector<std::shared_ptr<Usuario>> ObtenerUsuarios(const std::string& archivo)
    {
        ...
        while (std::getline(inputFile, line))
        {
            ...
            ss >> _id >> _nombre >> _correo >> _password >> _rol;

            usuarios.push_back(
                std::make_shared<Usuario>(_id, _nombre, _correo, _password, _rol)
            );
        }
    ...
    static bool RegistrarUsuario(const std::string& archivo,
        size_t _id, const std::string& _nombre,
        const std::string& _correo, const std::string& _password,
        const std::string& _rol)
    {
        std::ofstream outputFile(archivo, std::ios::app);
        if (!outputFile.is_open()) return false;

        outputFile << _id << " " << _nombre << " " << _correo << " "
            << _password << " " << _rol << std::endl;
